...
from qpth_plus.qpth.util import get_sizes, bdiag
...
def factor_solve_kkt(Q, D, G, A, rx, rs, rz, ry):
    ...
    ds = v_[:, nz:]
    dz = w_[:, :nineq]
    dy = w_[:, nineq:] if neq > 0 else None   # JK looks like dy are for eq constraints (nu) and dz are for ineq (lamb)
...
def solve_kkt(Q_LU, d, G, A, S_LU, rx, rs, rz, ry):
    ...
    ds = g2 / d
    dz = w[:, neq:]
    dy = w[:, :neq] if neq > 0 else None    # JK looks like dy hold the eq info (nu) and dz hold the ineq info (lamb)
