...
# JK 0712 adapted from QPFunction
# returns primal and dual variables in a single tensor
# [x,lamb,nu]
# along with gradients
# Amos+Kolter eqn (7) has revised RHS in backward pass
def QPFunction_Plus(eps=1e-12, verbose=0, notImprovedLim=3,
    ...
    class QPFunctionFn(Function):
        ...
        def forward(ctx, Q_, p_, G_, h_, A_, b_):
            ...
            if check_Q_spd:
                for i in range(nBatch):
                    e_complex = torch.linalg.eig(Q[i]).eigenvalues
                    e = torch.stack((e_complex.real, e_complex.imag), dim=-1)
            ...
            ####
            #print("zhats.shape = ")
            #print( zhats.shape if zhats!=None else None )
            #print("lams.shape = ")
            #print( ctx.lams.shape if ctx.lams!=None else None  )
            #print("nus.shape = ")
            #print( ctx.nus.shape if ctx.nus!=None else None   )
            sol_list = []
            if zhats != None:
                sol_list.append(zhats)
            if ctx.lams!= None:
                sol_list.append(ctx.lams)
            if ctx.nus!= None:
                sol_list.append(ctx.nus)

            full_soln = torch.cat( tuple(sol_list),1 )
            #full_soln = torch.cat( (zhats,ctx.lams,ctx.nus),1 )
            return full_soln
            ####
            #return zhats
        ...
        def backward(ctx, dl_dzhat):
            ...
            ###### JK 0712
            # dl_dzhat is formerly the loss grad wrt zhat
            # temp - now is loss grad wrt [zhat,lams,nus]
            ...
            #print( dl_dzhat.shape )
            dl_dx   = dl_dzhat[:,:ctx.nz]
            dl_dlam = dl_dzhat[:,ctx.nz:ctx.nz+ctx.nineq]
            dl_dnu  = dl_dzhat[:,ctx.nz+ctx.nineq:ctx.nz+ctx.nineq+ctx.neq]
            ...
            dx, _, dlam, dnu = pdipm_b.solve_kkt(
                ...
                dl_dlam,
                dl_dnu if neq > 0 else torch.Tensor())
...
# JK: adapted from QPFunction
# First goal: retrieve the Jacobian of the QP mapping rather than
#   just its matrix-vector product with incoming gradients
# UNTOUCHED
def QPFunction_Jacobian(eps=1e-12, verbose=0, notImprovedLim=3,
                 maxIter=20, solver=QPSolvers.PDIPM_BATCHED,
                 check_Q_spd=True):
    class QPFunctionFn(Function):
        @staticmethod
