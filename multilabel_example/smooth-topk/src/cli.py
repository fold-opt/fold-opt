...
def set_defaults(args):
    ...
    #modify save_name 
    args.out_name = os.path.join(args.out_name, args.dataset, "loss{}_top{}_epochs{}_samples{}".format(args.loss, args.topk, args.epochs, args.train_size))
    if not os.path.exists(os.path.dirname(args.out_name)): 
        os.makedirs(os.path.dirname(args.out_name))
...
def _add_loss_parser(parser):
    ...
    l_parser.add_argument('--loss', type=str, required=True,
                          choices=['svm', 'ce', 'svm-lapin', 'lml', 'ml', 'entr', 'pgd', 'cvx', 'sqp', 'fpgd'],
    ...
    l_parser.add_argument('--num_steps', type=int, default=60,
                      help="num iteration steps for pgd loss")
...
def _add_misc_parser(parser):
    ...
    m_parser.set_defaults(visdom=False, log=True, debug=False, eval=False)
